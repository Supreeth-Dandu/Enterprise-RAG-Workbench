# to load docs, chuck, embed and store in vectorDB
import os  # to access environment variables like API keys
from langchain_community.document_loaders import TextLoader, DirectoryLoader  
from langchain_text_splitters import CharacterTextSplitter  
from langchain_openai import OpenAIEmbeddings  
from langchain_chroma import Chroma  
from dotenv import load_dotenv  # Loads environment variables(OPENAI_API_KEY) from a .env file

load_dotenv() # Activate environment variables
...
def load_documents(docs_path="docs"):
    ...
    loader = DirectoryLoader(           # class from LC
        path=docs_path,
        glob="*.txt",               #telling the class to only look for txt files
        #loader_cls=TextLoader
        loader_cls=lambda path: TextLoader(path, encoding="utf-8")

    )
    
    documents = loader.load()   # we're invoking the load method once the loader has been configured
     # this is going to have the list of lC docs (like shown in the bottom comments)
...
def split_documents(documents, chunk_size=1000, chunk_overlap=0):
    ...
    text_splitter = CharacterTextSplitter(  # the basic text-splitting class in LC
    ...
    #     for i, chunk in enumerate(chunks[:5]):  #prints limited to 1st 5 chunks
    #         print(f"\n--- Chunk {i+1} ---")
...
def create_vector_store(chunks, persist_directory="db/chroma_db"):  # pssing the chunks, where we want to create & store the vectorDB locally
    ...
    vectorstore = Chroma.from_documents(
        ...
        persist_directory=persist_directory, # to store 
        collection_metadata={"hnsw:space": "cosine"}  # algo to retrive best chunks: cosine similarity
    )
